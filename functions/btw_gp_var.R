...
# FUNCTION: Compute Between-Group Variance (BGV)
...
btw_gp_var <- function(data, prop, period, age.lab = "0-5") {
        ...
        var <- lapply(region, 
                     function(x) {(prop$prop[prop$region == x]/100)*(reg_est$mean[reg_est$region == x] - nat_est)^2})
        
        return(list(sum(unlist(var)), period, age.lab) )
