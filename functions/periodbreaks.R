...
# Notes:
# 1) set common period breaks but only keep three periods for each DHS
# 2) amount.info allows to define if 1 year of data is sufficient to consider a period

#-------------------------------------------------------------

periodbreaks <- function(data, nber.p = 10, length.p = 4, amount.info = 1) {
        ...
        upper <- ifelse(abs(enddate_DHS - floor(enddate_DHS)) == 0, enddate_DHS, # if enddate_DHS after September (3/4 of a year) use it as highest year
                        ifelse(abs(enddate_DHS - floor(enddate_DHS)) >= 0.75, floor(enddate_DHS) + 1,
                               floor(enddate_DHS)))
        lower <- upper - (nber.p*length.p) # defines lowest year for year.cut
